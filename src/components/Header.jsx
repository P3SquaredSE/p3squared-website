  ...
  return (
    <header className="fixed top-0 left-0 z-50 w-full bg-[#0B3356] shadow-md">
      ...
      <div className="container mx-auto flex items-center justify-between px-6 py-4">
        ...
        <nav
          className="hidden lg:flex items-center gap-10 text-white/90 text-sm font-medium"
          aria-label="Primary"
        >
          {navLinks.map((link) => (
            <a
      ...
      {isOpen && (
        <div
          id={menuId}
          className="lg:hidden bg-[#0B3356] border-t border-white/10"
        >
          <nav
            aria-label="Mobile"
            className="flex flex-col px-6 py-4 gap-4 text-white/90 text-base"
          >
            {navLinks.map((link) => (
              <a
                key={link.href}
